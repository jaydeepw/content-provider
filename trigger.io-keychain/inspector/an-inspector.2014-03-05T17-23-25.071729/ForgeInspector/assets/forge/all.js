...
forge['keychain'] = {
	...
	'get': function (key, success, error) {
		forge.internal.call("keychain.get", {
			key: key.toString()
	...
	'set': function (key, value, success, error) {
		...
		forge.internal.call("keychain.set", {
			key: key.toString(),
			value: value
		}, success, error);
	},
	...
	'keys': function(success, error) {
		forge.internal.call("keychain.keys", {}, success, error);
	},
	/**
	...
	'all': function(success, error) {
		var success = success && function (keychain) {
			for (key in keychain) {
				if (keychain[key] === "undefined") {
					keychain[key] = undefined;
				} else {
					keychain[key] = JSON.parse(keychain[key]);
				}
			}
			success(keychain);
		}
		forge.internal.call("keychain.all", {}, success, error);
	},
	/**
